...
class BG():
    ...
    # 功能：分段
    def FD_data(self,data_li, fd_len):
        '''
    ...
    # 数据处理
    def CH(self, df, filename, header, mo):
        '''
        处理数据
        :param df: df对象|
    ...
    # 功能：保存数据的整个过程| 对数据进行处理成统一格式-》是否可以需要分段-》保存文件
    def To_data(self,filename,data,mo,max_num=0):
        '''
        :param filename: 文件名|str|
        ...
        :return:
        ps: 1.xls单个sheet限制：65,535条记录 2.xlsx单个sheet限制：1,048,576条记录 3.csv 单个sheet无限制
        ...
        if len(new_data) < max_num:  # 如果数据长度小于最大长度，直接保存文件
            ...
            self.CH(new_df, filename, header, mo)
        elif len(new_data) >max_num and mo != 'a':   # 如果数据长度大于最大长度，分段后保存文件(追加模式下不能分段)
            FD_data = self.FD_data(new_data, max_num)
            for fd_num in range(0, len(FD_data)):
                new_filename = filename[0:filename.rfind('.')] + '-' + str(fd_num + 1) + filename[filename.rfind('.'):]  # xx.csv -> xx-1.csv
                new_df = pd.DataFrame(FD_data[fd_num])
                self.CH(new_df, new_filename, header, mo)
        else:
...
if __name__ == '__main__':
    ...
    # dic=[{'name1':'aaa'},{'name1':'bbb'},{'name1':'ccc'},{'name1':'ddd'},{'name1':'sss'}]
    dic=[{'name1':'aaa%d'%i} for i in range(0,65545)]
    # 创建对象并写入
    bg=BG()
    for i in range(1,3):
        # bg.To_data('xx/di.csv',di,'w')
        # bg.To_data('xx/di.xls', di, 'w')
        # bg.To_data('xx/di.xlsx', di, 'w')
        # bg.To_data('xx/li.csv', li, 'w')
        # bg.To_data('xx/li.xls', li, 'w')
        # bg.To_data('xx/li.xlsx', li, 'w')
        bg.To_data('xx/dic.csv', dic, 'w')
        bg.To_data('xx/dic.xls', dic, 'w')
        bg.To_data('xx/dic.xlsx', dic, 'w')
